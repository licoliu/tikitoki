...
TLConfigText['basic_daySuffixes'] = [ "st", "nd", "rd", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "st", "nd", "rd",
...
TLConfigText['marker_moreButton_text'] = "More";
TLConfigText['imageUploader_title'] = "Upload image";
TLConfigText['imageUploader_intro'] = 'Choose an image from your hard drive. Your account gives you <span id="tl-image-uploader-limit">x1</span> uploaded images per timeline. You can add <span id="tl-image-uploader-remaining">x1</span> more uploaded images to this timeline. Only upload images you have permission to use.';
TLConfigText['imageURLSelect_title'] = "Image url";
TLConfigText['imageURLSelect_intro'] = 'Enter the url of an image on the web and click the \'Select\' button. This is the best option if you want to eventually host the timeline on your own webserver as part of our Single Timeline License.';
TLConfigText['3d_click_to_find_out_more'] = "Click to find out more";
...
var AJKHelpers = {
	...
	getProcessedImageUrlForAccount : function(data) {
		...
		if (imgSrc && imgSrc.indexOf("lico.atom.com") != -1) {
			...
			if (accountType == "Standard" && !forceShow) {
				...
				return imgSrc.replace("www." + this.uploadDomain, this.cloudfront).replace(this.uploadDomain, this.cloudfront);
			}
			*/
		} else {
			return imgSrc;
	...
	generateAudioEmbedHTML : function(data) {
		...
		if (type == "dailymotion" || type == "youtube" || type == "vimeo") {
			...
			if ($.browser.msie || $.browser.isModernIE) {
				...
				iHTML += '<param name="movie" value="http://player.soundcloud.com/player.swf?url=' + audioSrc + '&auto_play=true&show_artwork=true&theme_color=000000&color='
						+ colour + '"></param>';
				...
				iHTML += '<embed allowscriptaccess="always" height="100%" src="http://player.soundcloud.com/player.swf?url=' + audioSrc
						+ '&auto_play=true&show_artwork=true&theme_color=000000&color=' + colour + '&visual=true" type="application/x-shockwave-flash" width="100%"></embed>';
				iHTML += '</object>';
			} else {
	...
	generateVideoEmbedHTML : function(data) {
		...
		switch (type) {
		case "vimeo":
			insertHTML = '<iframe src="' + protocal + '//player.vimeo.com/video/' + videoId + '?color=' + colour + '&autoplay=1&title=0&portrait=0&byline=0" width="' + width
					+ '" height="' + height + '" frameborder="0"></iframe>';
			break;
		case "youtube":
			if ($.browser.msie && $.browser.version < 9) {
				insertHTML = '<embed src="http://www.youtube.com/v/' + videoId + '" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="'
			...
			break;
		case "dailymotion":
			insertHTML += '<iframe src="' + protocal + '//www.dailymotion.com/embed/video/' + videoId + '" width="' + width + '" height="' + height + '" frameborder="0"></iframe>';
			break;
		case "skoletube":
	...
	generateStandardVideoPlayerEmbedHTML : function(data) {
		...
		if (this.isIDevice()) {
			...
			insertHTML += '<embed type="application/x-shockwave-flash" width="100%" height="100%" src="/assets/jaris-player/player.swf?1.002" allowfullscreen="true" bgcolor="#'
					+ backColor + '" flashvars="source=' + mediaUrl + '&streamtype=file&type=' + mediaType + '&autostart=true&darkcolor=' + backColor + '&brightcolor=' + backColor
					+ '&controlcolor=' + controlColor + '&hovercolor=67A8C1&controltype=1">';
		} else {
			...
			insertHTML += '<param name="flashvars" value="source=' + mediaUrl + '&streamtype=file&type=' + mediaType + '&duration=52&darkcolor=' + backColor + '&brightcolor='
	...
	customCreateClickableLinks : function(data) {
		...
		return aString.replace(myRegex, function() {
			extra = (AJKHelpers.isUrlToCurrentPage({
				url : RegExp.$2
			})) ? "" : extra;
	...
	createDateWithTime : function(data) {
		var retDate = new Date();
	...
	baseLangDateDaySuffixArray : [ "st", "nd", "rd", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "th", "st", "nd", "rd", "th",
			"th", "th", "th", "th", "th", "th", "st", "nd", "rd", "th", "th", "th", "th", "th", "th", "th" ],
	baseLangBC : "BC",
	baseLangAD : "AD",
	...
	twelveHourHour : function(data) {
		var hour = data.hour;
		var hour = hour % 12;
		hour = (hour == 0) ? 12 : hour;
		return hour;
	},
	amOrPmForHour : function(data) {
		var hour = data.hour;
		return (hour > 11) ? TLConfigText['basic_pm'] : TLConfigText['basic_am'];
	},
	...
	prepareGTLTForText : function(data) {
		var content = data.content;
...
var TLHashController = function() {
	var self = this;
	self.lastHash = "";
	self.timeout = "";
	self.timeoutTime = 200;
};
TLHashController.prototype = {
	init : function() {
		var self = this;
	...
	currentDateWasUpdatedTo : function(data) {
		...
		self.lastHash = AJKHelpers.formatDate({
			date : aDate,
		...
		if (splitHash && splitHash[1]) {
			...
			if (info.indexOf("date=") != -1) {
				...
				if (dateTxt) {
					...
					aDate = AJKHelpers.dateFromMySQLDate({
						dateString : dateTxt
					});
				}
			}
		...
		if (animate && !instantly) {
			$({
				...
				step : function() {
					...
					self.currentDate = AJKHelpers.cloneDate({
						date : aDate
					});
					self.informObservers({
						extraInfo : extraInfo
...
TLSliderController.prototype = {
	...
	calibrateScale : function() {
		...
		theTLSettings.visibleStartDate = AJKHelpers.createDateWithTime({
			time : vStartDate
		});
		theTLSettings.visibleEndDate = AJKHelpers.createDateWithTime({
			time : vEndDate
		});
	...
	initialiseDragger : function() {
		...
		self.dragger = new TLSliderDragger({
			...
			dragCallback : function(data) {
				...
				var newDate = AJKHelpers.createDateWithTime({
...
var TLMainController = function(dataObj) {
	...
	self.timeline = {
		...
		title : AJKHelpers.removeScript({
			content : TLTimelineData.title
		}),
		authorName : AJKHelpers.removeScript({
		...
		introText : AJKHelpers.removeScript({
			content : TLTimelineData.introText
		}),
		aboutText : AJKHelpers.removeScript({
			content : ''//TODO $(self.domMainHolder).find(".tl-timeline-info p.tl-ah-about-text").html().replace(/;xNLx;/g, "\n")
		}),
...
TLMainController.prototype = {
	init : function() {
		...
		AJKHelpers.isScriptable = self.timeline.scriptable;
		self.sortCategories();
	...
	initTimelineControls : function() {
		var self = this;
		self.userControlsController = new TLUserControls().init();
		//TODO
		self.userChartsController = new TLUserCharts("#tl-uc-messages").init();
		self.userChartsController = new TLUserCharts("#tl-uc-charts").init();
	},
	...
	initLanguage : function() {
		...
		if (trans) {
			...
			AJKHelpers.dateBillion = trans.billion;
			AJKHelpers.dateMillion = trans.million;
			AJKHelpers.dateAD = trans.AD;
			AJKHelpers.dateBC = trans.BC;
			AJKHelpers.dateWeekDayArray = trans.weekDays;
			AJKHelpers.dateWeekDayShortArray = trans.shortWeekDays;
			...
			TLConfigText['3d_click_to_find_out_more'] = trans.clickToFindOutMore;
	...
	refreshTimelineAfterFeedChange : function() {
		...
		if (self.markers.length > 1 && self.timeline.feeds.length > 0) {
			self.timeline.startDate = AJKHelpers.createDateWithTime({
				time : self.markers[0].startDate.getTime()
			});
			self.timeline.endDate = AJKHelpers.createDateWithTime({
				time : self.markers[self.markers.length - 1].startDate.getTime()
			});
			var origStartDate = AJKHelpers.dateFromMySQLDate({
				dateString : TLTimelineData.startDate
			});
			var origEndDate = AJKHelpers.dateFromMySQLDate({
				dateString : TLTimelineData.endDate
			});
	...
	initialise3DView : function() {
		var self = this;
		self.selected3DView = new TL3DViewController({
			items : self.markers,
	...
	initialiseCategory : function(data) {
		...
		category.title = AJKHelpers.removeScript({
...
TL3DViewController.prototype = {
	...
	initialise : function() {
		...
		$(self.domCanvas).click(function(e) {
			...
			if (TLGLOBALIsTouchDevice) {
				var mousePos = AJKHelpers.getMousePos({
					event : e
				});
			...
			self.startStageDragPos = AJKHelpers.getMousePos({
				event : e
			});
			self.startStageDragDate = AJKHelpers.cloneDate({
				date : theTLSettings.currentDate
			});
	...
	easeViewToDate : function(data) {
		var self = this;
		self.desiredDate = AJKHelpers.cloneDate({
			date : data.date
		});
		var instantly = data.instantly;
		if (instantly) {
	...
	moveViewToDate : function(data) {
		var self = this;
		var cDate = AJKHelpers.cloneDate({
			date : data.date
		});
	...
	resizeContent : function(data) {
		...
		self.canvasVert = AJKHelpers.getCoordsOfDomEl({
	...
	drawView : function() {
		...
		var tcRGB = AJKHelpers.convertHexColourToRGB({
		...
		var bgRGB = AJKHelpers.convertHexColourToRGB({
		...
		if (catView) {
			for (var cat = 0; cat < numCols; cat++) {
				...
				var catRGB = AJKHelpers.convertHexColourToRGB({
	...
	drawMarkerOnScreenAtTimelinePos : function(data) {
		...
		if (self.timeline.viewType == "color-category-stories" || self.timeline.viewType == "duration") {
			...
			var cRGB = AJKHelpers.convertHexColourToRGB({
				hexColour : marker.category.colour
			});
		...
		if ((self.selectedMarker == marker || self.hoverMarker == marker) && marker.marker3DScreenInfo.timelinePos.y > self.moreInfoCutOff && self.timeline.lightboxStyle != 2) {
			...
			ctx.textAlign = "left";
			ctx.fillText(TLConfigText['3d_click_to_find_out_more'].toUpperCase(), blockX + clickBlock.left, blockY + (clickBlock.height) - clickBlock.bottom);
		}
		var startPos = {
			y : screenPos.y + vShift,
			x : screenPos.x
...
TLViewController.stageDragMultiplier = 1;
...
TLViewController.prototype = {
					...
					if (domStoryBlock) {
						...
						if ($(e.target).hasClass("ig-inner-image-mask") && clickedStoryBlock.isFlickrImage) {
							if (!AJKDraggable.cancelClick) {
								...
								|| (self.timeline.viewType == "category-band" && clickedStoryBlock.category.viewType == "duration") || AJKHelpers.isIDevice()) {
	...
	createCategoryBands : function() {
		...
		var totalSize = 0;
		$.each(self.timeline.categories, function() {
			if (!this.hide && !(this.autoGenerated && theTLMainController.markersByCategory[this.key] && theTLMainController.markersByCategory[this.key].markers.length == 0)) {
		...
		//TODO
		var allSpace = (totalSize <= 100) ? 100 : totalSize;
		if(allSpace > 100){
			$(self.domEl).css({height: allSpace + "%"});
			$(theTLMainController.domFixedStageContent).css({height: allSpace + "%"});
		}
		...
		$.each(ourCategories, function() {
			...
			var bandTop = allSpace - spaceRemaining;
	...
	initialiseViewDrag : function() {
		...
		self.draggableObj = new AJKDraggable({
			multiplier : TLViewController.stageDragMultiplier,
			domDragEl : self.domEl,
			limitFunc : function() {
				var limit = {
	...
	createViewScaleBlocks : function() {
		...
		if (self.timeline.markerSpacing == "equal") {
			var insertHTML = '<div style="background: url(../assets/ui/stage/zebra-' + self.timeline.equalMarkerSpacing + 'px.png) '
					+ parseInt(self.timeline.equalMarkerLeftPadding - 0.5 * self.timeline.equalMarkerSpacing, 10) + 'px 0; width: ' + self.width + 'px;" class="scale-block">';
...
var TLViewScaleBlock = function(data) {
	...
	self.controller = data.controller;
	self.scaleGraphic = self.viewScale.segmentWidth ? '../assets/ui/stage/standard-scale-' + self.viewScale.segmentWidth + 'px-segments.png' : '../assets/ui/stage/scale-' + self.type
			+ '.png';
...
TLViewController.viewScaleSettings["hour-5-mins-medium"] = {
	...
	getFirstBlockStartDateFromDate : function(data) {
		var aDate = data.date;
		var retDate = AJKHelpers.createDateWithTime({
			time : aDate.getTime()
		});
	...
	getDateRangeInMS : function(data) {
		...
		var retDate = AJKHelpers.createDateWithTime({
			time : blockStartDate.getTime() + AJKHelpers.dateOneHourInMS
		});
		retDate.setMinutes(0);
		retDate.setSeconds(0);
		retDate.setMilliseconds(0);
...
TLViewController.viewScaleSettings["day-medium-hour"] = {
	...
	getDateRangeInMS : function(data) {
		var blockStartDate = data.blockStartDate;
		var retDate = AJKHelpers.createDateWithTime({
			time : blockStartDate.getTime() + AJKHelpers.dateOneDayInMS
		});
		retDate.setMinutes(0);
		retDate.setSeconds(0);
		return retDate.getTime() - blockStartDate.getTime();
	...
	getFirstBlockStartDateFromDate : function(data) {
		...
		if (aYear < 0) {
			if (aYear % 10 == 0) {
				...
				var roundedYear = parseInt(aYear / 10, 10) * 10 - 10;
			}
		} else {
...
TLViewController.viewScaleSettings["century-large-decade"] = {
	...
	getFirstBlockStartDateFromDate : function(data) {
		...
		if (aYear < 0) {
			if (aYear % 100 == 0) {
				...
				var roundedYear = parseInt(aYear / 100, 10) * 100 - 100;
			}
		} else {
			var roundedYear = parseInt(aYear / 100, 10) * 100;
		}
		retDate.setFullYear(roundedYear);
		retDate.setMonth(0);
		retDate.setDate(1);
	...
	getTextForDate : function(data) {
		...
		var fullYear = aDate.getFullYear();
		if (fullYear >= 0) {
			var roundedFullYear = parseInt((fullYear + 1) / 10, 10) * 10;
		} else {
			var roundedFullYear = parseInt((fullYear - 1) / 10, 10) * 10;
		}
...
TLViewController.viewScaleSettings["millenium-medium-century"].segmentWidth = 128;
(function() {
	var TLSCPDiv = 10000;
	var TLSCPMult = 2;
	...
	var TLSCPInterations = 30;
	var TLSCOMultipliers = [ 2, 5, 10 ];
	for (var counter = 0; counter < TLSCPInterations; counter++) {
		var scaleName = "large-date-range-" + counter;
		var scale = AJKHelpers.cloneObj({
			obj : TLSCPBlueprint
		});
		scale.largeDivider = TLSCOMultipliers[counter % 3] * Math.pow(10, parseInt(counter / 3, 10)) * TLSCPDiv;
...
var TLMarker = function(data) {
	...
	self.headline = AJKHelpers.removeScript({
		content : data.headline
	});
	self.id = data.id;
	self.dateFormat = AJKHelpers.removeScript({
	...
	self.fullText = AJKHelpers.removeScript({
...
TLMarker.prototype = {
	...
	generateFromToDateString : function(data) {
		...
		var endDateStr = AJKHelpers.formatDate({
			date : self.endDate,
			formatString : formatString
		});
	...
	generateTextImageFor3DForSize : function(data) {
		...
		var domCanvas = $(
				'<canvas style="width:' + size.width + 'px; height: ' + tHeight + 'px" width="' + (size.width * self.deviceScale) + '" height="' + (tHeight * self.deviceScale)
						+ '"></canvas>').get()[0];
		...
		self.lines3DText = AJKHelpers.renderCanvasText({
			canvasContext : ctx,
			text : titleText,
			textPos : {
				x : size.x,
	...
	generateSizedCanvas3DImage : function(data) {
		...
		var domCanvas = $('<canvas style="width:' + size.width + 'px; height: ' + size.height + 'px" width="' + size.width + '" height="' + size.height + '"></canvas>').get()[0];
		var ctx = domCanvas.getContext('2d');
	...
	focus : function() {
		...
		if ((self.timeline.viewType == "duration" || self.category.viewType == "duration") && self.startDate.getTime() != self.endDate.getTime()) {
			var sOffset = AJKHelpers.getCoordsOfDomEl({
...
TLContentPanelController.prototype = {
		...
		self.imageGallery = new AJKImageGallery({
			...
			createImageInfoHTML : function(data) {
				...
				if (anImage.caption) {
					...
					insertHTML += '<p>' + AJKHelpers.removeScript({
	...
	displayForStory : function(data) {
		...
		if (story.isTimelineIntro) {
			...
			$(self.domPanelDateText).text(TLConfigText['basic_By'] + " " + self.timeline.authorName);
		} else {
		...
		if (self.prevStory) {
			var pText = AJKHelpers.prepareGTLTForText({
		...
		if (self.nextStory) {
			var nText = AJKHelpers.prepareGTLTForText({
		...
		var headline = AJKHelpers.prepareGTLTForText({
...
var TLCPImageViewer = function(data) {
	var self = this;
	self.controller = data.controller;
	self.timeline = self.controller.timeline;
...
TLCPImageViewer.prototype = {
	...
	launchWithImage : function(data) {
		...
		var startPos = AJKHelpers.getCoordsOfDomEl({
		...
		if (objImage.caption) {
			$(self.domCaption).css({
				display : "block"
			}).find("p").empty().append(AJKHelpers.removeScript({
	...
	restrictImageSizeToScreen : function(data) {
		...
		var padding = 40;
		var imageSize = data.imageSize;
		var screenSize = data.screenSize || AJKHelpers.viewportSize();
	...
	hide : function(data) {
		...
		if ($.browser.msie || self.timeline.expander != 2) {
			...
			var endPos = AJKHelpers.getCoordsOfDomEl({
				domEl : self.imageGallery.domStageHolder
			});
			$(self.domImage).animate({
...
var AJKValueSlider = function(data) {
	var self = this;
	self.domRoot = data.domRoot;
...
AJKValueSlider.slidersByName = [];
AJKValueSlider.prototype = {
	init : function() {
		var self = this;
		AJKValueSlider.slidersByName[self.name] = self;
		self.dragger = new AJKDraggable({
...
AJKVerifierField.prototype = {
	...
	setupValueGetterSetter : function() {
		...
		case "textarea textarea":
			self.getValue = function() {
				return AJKHelpers.removeScript({
					content : $(this.domControl).val()
				});
			};
			self.setValue = function(data) {
				$(this.domControl).val(AJKHelpers.removeScript({
					content : data.value
				}));
			};
			break;
		...
		if (self.stageHeight > self.carouselHeight) {
			...
			if (!self.scrollDragInitialised) {
				...
				if (typeof TLGLOBALIsTouchDevice != "undefined" && TLGLOBALIsTouchDevice) {
					var contentDraggable = new AJKDraggable({
...
$(document).ready(function() {
	...
	theTLSettings = new TLSettings().init();
	theTLHashController = new TLHashController().init();
	(function() {
	...
	e.click(function(){
		$("#tl-stage-holder").animate({scrollTop:0},"slow","linear");
    });
